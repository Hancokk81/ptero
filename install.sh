...
display_welcome() {
  ...
  echo -e "${BLUE}[+]                  © NEOXLOUD OFFICIAL                [+]${NC}"
  echo -e "${BLUE}[+]                                                 [+]${NC}"
  echo -e "${RED}[+] =============================================== [+]${NC}"
  echo -e ""
...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSE NEOXLOUD OFFICIAL            [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKKAN AKSES TOKEN: ${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "nazwa" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di NeoXloud Official${NC}"
    echo -e "${YELLOW}TELEGRAM : @neoofc29${NC}"
    echo -e "${YELLOW}WHATSAPP : +62895364952866${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© NEOXLOUD OFFICIAL${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/Hancokk81/ptero/raw/main/arix.zip")  
        break
        ;;
      2)
...
# Unduh file tema
wget -O /root/arix.zip https://github.com/Hancokk81/ptero/raw/main/arix.zip


# Ekstrak file tema
unzip /root/arix.zip -d /root/pterodactyl
...
while true; do
  ...
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : @neoofc29      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Whatsapp  : +62895364952866  ${NC}"
